...
### Fixed
- Reject dependency cycles and self-dependencies in `urirun:processes` plans
  before execution, with a deterministic cycle path in the validation error.

### Changed
- Document that wildcard URI patterns are authorization scopes and executable
  process addresses must always be concrete.
